...
<head>
    ...
    <title>Banner Page 3 Control Panel</title>
...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $bannerpage3_Id = $_POST['bannerpage3_Id'];
        $action = $_POST['action'];

        try {
            switch ($action) {
                case 'deactivate':
                    $deactivatequery = "UPDATE bannerpage3 SET status = 'inactive' WHERE bannerpage3_Id = :bannerpage3_Id";
                    $deactivatestmt = $conn->prepare($deactivatequery);
                    $deactivatestmt->bindParam(':bannerpage3_Id', $bannerpage3_Id, PDO::PARAM_INT);
                    $deactivatestmt->execute();
                    break;
                case 'activate':
                    $activatequery = "UPDATE bannerpage3 SET status = 'active' WHERE bannerpage3_Id = :bannerpage3_Id";
                    $activatestmt = $conn->prepare($activatequery);
                    $activatestmt->bindParam(':bannerpage3_Id', $bannerpage3_Id, PDO::PARAM_INT);
                    $activatestmt->execute();
                    break;
                case 'delete':
                    $expirequery = "UPDATE bannerpage3 SET expired = 'yes' WHERE bannerpage3_Id = :bannerpage3_Id";
                    $expirestmt = $conn->prepare($expirequery);
                    $expirestmt->bindParam(':bannerpage3_Id', $bannerpage3_Id, PDO::PARAM_INT);
                    $expirestmt->execute();
                    break;
            }
    ...
    try {
        $stmt = $conn->prepare("SELECT * FROM bannerpage3 WHERE expired <> 'yes';");
        $stmt->execute();
        ...
        if ($result) {
            ...
            echo '<div>';
            echo '<h1 class="page-title">Control Banner Page 3</h1>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            ...
            foreach ($result as $row) {
                ...
                echo "<td>
                        <form method='post' action='bannerpage3.php'>
                            <input type='hidden' name='bannerpage3_Id' value='" . htmlspecialchars($row['bannerpage3_Id']) . "' />
                ...
                echo '<form method="post" action="bannerpage3.php">';
                echo "<td>
                        <form method='post' action='bannerpage3.php'>
                            <input type='hidden' name='bannerpage3_Id' value='" . htmlspecialchars($row['bannerpage3_Id']) . "' />
